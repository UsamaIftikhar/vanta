...
- Production-flow follow-up corrected an extra contact-form markup delimiter and added browser coverage for contact resources, blog rendering and the native cart-to-Shopify-checkout handoff. Build, structural tests, Theme Check and dependency audit pass. The refreshed authenticated browser run and a Shopify test-mode payment remain external gates because the local preview requires the storefront password.
